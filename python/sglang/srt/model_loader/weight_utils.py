...
def initialize_dummy_weights(
    ...
    share_dummy_moe = os.getenv("SGLANG_DUMMY_SHARE_MXFP8_MOE_WEIGHTS", "0") == "1"
    # keep_vars preserves Parameter-side layout markers such as format_ue8m0.
    for param in model.state_dict(keep_vars=True).values():
        if share_dummy_moe:
            storage_key = (param.device, param.data_ptr())
            if storage_key in initialized_storages:
                continue
            initialized_storages.add(storage_key)
