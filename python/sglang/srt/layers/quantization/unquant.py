...
# Qwen3.5 TP16 tactics measured on GB300 under CUDA graph replay with PDL
...
def initialize_bf16_gemm_config(server_args: ServerArgs) -> None:
    ...
    _enable_bf16_splitk_gemm = should_enable_bf16_splitk_gemm(backend)
    if _enable_bf16_splitk_gemm:
        ...
        # Load only the new standalone direct kernel from FlashInfer PR #4266.
        # Importing the whole PR source tree would replace the image's matching
        # attention Python package while retaining its older AOT/JIT binaries.
        # That mixture is ABI-incompatible with TRTLLM paged attention.
        source_root = os.environ.get("SGLANG_FLASHINFER_PR4266_SOURCE")
        if not source_root:
            raise ValueError(
                "SGLANG_FLASHINFER_PR4266_SOURCE must point to a FlashInfer "
                "checkout containing PR #4266"
            )
        direct_path = os.path.join(
            source_root,
            "flashinfer/gemm/kernels/dense_bf16_gemm_direct.py",
        )
        spec = importlib.util.spec_from_file_location(
            "_sglang_flashinfer_pr4266_direct", direct_path
        )
        if spec is None or spec.loader is None:
            raise ImportError(f"Cannot load FlashInfer PR #4266 kernel: {direct_path}")
        direct_module = importlib.util.module_from_spec(spec)
        sys.modules[spec.name] = direct_module
        spec.loader.exec_module(direct_module)
        _flashinfer_pr4266_direct_default_tactic = direct_module.default_tactic
        _flashinfer_pr4266_prefer_direct = (
            direct_module.prefer_direct_bf16_gemm_sm100
        )
        _flashinfer_pr4266_run_direct_dense = direct_module.run_direct_dense
